...
if ($form.name =="save_multi_descr") {
       var elems = document.querySelectorAll('input[name="multi_descr[]"]:checked');
    ...
    if(ajaxReq.status == 200) {
     ...
     if ($response) { 
      if ($response.status == "autorized") {
       ...
       if ( $href=='/login') {
         ...
         $href='/';
          ...
          $btn_login = id('logout');        
          $btn_login.classList.remove('inviz');
      ...
      if ($response.notify) {       
        ...
        if ($response.status == "multi_descr_ok") {
           close_multi_descr();
        } 
...
function close_multi_descr(){
        id('multi_descr').parentNode.removeChild( id('multi_descr'));
            var elems_check = document.querySelectorAll('input[name="multi_descr[]"]:checked');
            for (var i = 0; i < elems_check.length; i++) {
          
                 elems_check[i].checked=false; 
            }
            $elems_check=$panel_multi_descr=$multi_descr='';
}
...
function remove_img($target){
  ...
  post('ajax.php', params, function($response) {
     ...
     if ($resp.status=="file_delete") {
        $img = parent_by_selector($target, 'thumb');
         ...
         var $resp=JSON.parse($response);   
          if ($resp.status = 'category_changed') {
            ...
            // elems[i].checked = false;
             ...
             if ($resp.notify) {       
                   if ($resp.notify_style) {
                     notify($resp.message, $resp.notify_style);
                   } else notify($resp.message);
                 } 
       });
...
function multi_change(){
  if ($panel_category =='') {
    window.$panel_category =  id('right-box'); 
  }
...
var $panel_multi_descr='', $multi_descr='';
function multi_descr(target){
  if ($panel_multi_descr =='') {
    window.$panel_multi_descr =  id('right-box'); 
  }

if ($multi_descr=='') {      
    var $data = new FormData();
        $data.append("action", "multi_descr"); 
         $data.append("product", target.value);       
        post_z('ajax.php',  $data, function($response) {
    var $resp=JSON.parse($response);         
       window.$multi_descr=$resp.content;       
       $panel_multi_descr.insertAdjacentHTML('beforeend', $multi_descr);      
     });      
} 
  if ($multi_descr!='' && window.$panel_multi_descr.innerHTML=='') {
    $panel_multi_descr.insertAdjacentHTML('beforeend', $multi_descr);  
  }  
}
...
var $row=parent($target, 'tr');
    ...
    if ($resp.content) {
      ...
      if (txt=document.forms[$id].description) {
      var htext = txt.scrollHeight;
           txt.style.height = htext + "px";
           txt=htext=null;
          // document.forms[$id].description.height = "auto";
        } 
      }
    }
...
function delete_propose(target){
var $id = target.getAttribute('href');
// var $action = target.id;
var $data = new FormData();
  ...
  post_z('../ajax.php',  $data, function($response) {
        if ($response) {         
         $resp=JSON.parse($response); 
          if ($resp.status=='propose_deleted_ok') {
...
function hide_propose(target){
var $id = target.getAttribute('href');
// var $action = target.id;
  var $data = new FormData();

  $data.append("action", 'hide_propose');
  $data.append("id", $id);
  console.log( $data);
  post_z('../ajax.php',  $data, function($response) {
        if ($response) {         
         $resp=JSON.parse($response); 
          if ($resp.status=='propose_hided_ok') {
      var $propose_panel =  id('propose_'+$id);
          $propose_panel.parentNode.removeChild($propose_panel);
          $propose_panel =  null;
          }
...
function edit_propose(target){
   notify('Временно не работает');
...
function send_in_vk($target){
  $id_prod = $target.getAttribute('href');
  $action=$target.id;
  var $data = new FormData();
      $data.append("action", $action);
      $data.append("id", $id_prod);
      post_z('../ajax.php',  $data, function($response) {
         if ($response) {  
          $resp=JSON.parse($response); 
...
function logout(){
  var $data = new FormData();
    ...
    post_z('../ajax.php',  $data, function($response) {
      ...
      $btn_logout.classList.add('inviz');
...
//       } else if (img_t.width<=300){
//       console.log(img_t.src);
//           $canvas_thumb.width = img_t.width;
//           $canvas_thumb.height = img_t.height;
//       var ctx = $canvas_thumb.getContext("2d");
//           ctx.drawImage(img_t, 0, 0);
//           thumb =  $canvas_thumb.toDataURL($type);   
//       }
...
//           // console.log(this);
//           $list.insertBefore(div, null);             
//           div.innerHTML = ['<div class="remove"><a class="test" id="remove_img" data-filename="'+data_files.remov_link+'"><img src="template/img/close.gif" alt="" /></a></div><img class="thumb" src="upload/thumbs/',  data_files.file_name, '" title="thumb_', escape($file_name), '"/>'].join('');          
//           });
